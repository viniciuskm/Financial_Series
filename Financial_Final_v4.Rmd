...
cor(petr_filtered,ibov_filtered)
cor(vale_filtered,ibov_filtered)
cor(vvar_filtered,ibov_filtered)
cor(cmig_filtered,ibov_filtered)
  ...
  for (j in 1:max_ordem){
    for (l in 1:length(dist)){
      for (m in 1:length(model)){
        ...
        fit_petr_mais[[k]] = ugarchfit(specs_petr_mais[[k]], data = petr_filtered+ibov_filtered, solver = 'hybrid')

        fit_vale_mais[[k]] = ugarchfit(specs_vale_mais[[k]], data = vale_filtered+ibov_filtered, solver = 'hybrid')

        fit_vvar_mais[[k]] = ugarchfit(specs_vvar_mais[[k]], data = vvar_filtered+ibov_filtered, solver = 'hybrid')

        fit_cmig_mais[[k]] = ugarchfit(specs_cmig_mais[[k]], data = cmig_filtered+ibov_filtered, solver = 'hybrid')
        
        fit_ibov[[k]] = ugarchfit(specs_ibov[[k]], data = ibov_filtered, solver = 'hybrid')
        
        fit_petr_menos[[k]] = ugarchfit(specs_petr_menos[[k]], data = petr_filtered-ibov_filtered, solver = 'hybrid')

        fit_vale_menos[[k]] = ugarchfit(specs_vale_menos[[k]], data = vale_filtered-ibov_filtered, solver = 'hybrid')

        fit_vvar_menos[[k]] = ugarchfit(specs_vvar_menos[[k]], data = vvar_filtered-ibov_filtered, solver = 'hybrid')

        fit_cmig_menos[[k]] = ugarchfit(specs_cmig_menos[[k]], data = cmig_filtered-ibov_filtered, solver = 'hybrid')
        ...
        BICs_petr_mais[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_petr_mais[[k]])[2],3)))
        BICs_vale_mais[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_vale_mais[[k]])[2],3)))
        BICs_vvar_mais[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_vvar_mais[[k]])[2],3)))
        BICs_cmig_mais[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_cmig_mais[[k]])[2],3)))
        BICs_ibov[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_ibov[[k]])[2],3)))

        BICs_petr_menos[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_petr_menos[[k]])[2],3)))
        BICs_vale_menos[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_vale_menos[[k]])[2],3)))
        BICs_vvar_menos[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_vvar_menos[[k]])[2],3)))
        BICs_cmig_menos[k,] = c(i,j, model[m], dist[l],  abs(round(infocriteria(fit_cmig_menos[[k]])[2],3)))
...
best_petr_mais <- which(BICs_petr_mais[,5] == max(BICs_petr_mais[,5]), arr.ind = TRUE)
best_vale_mais <- which(BICs_vale_mais[,5] == max(BICs_vale_mais[,5]), arr.ind = TRUE)
best_vvar_mais <- which(BICs_vvar_mais[,5] == max(BICs_vvar_mais[,5]), arr.ind = TRUE)
best_cmig_mais <- which(BICs_cmig_mais[,5] == max(BICs_cmig_mais[,5]), arr.ind = TRUE)
best_ibov <- which(BICs_ibov[,5] == max(BICs_ibov[,5]), arr.ind = TRUE)

best_petr_menos <- which(BICs_petr_menos[,5] == max(BICs_petr_menos[,5]), arr.ind = TRUE)
best_vale_menos <- which(BICs_vale_menos[,5] == max(BICs_vale_menos[,5]), arr.ind = TRUE)
best_vvar_menos <- which(BICs_vvar_menos[,5] == max(BICs_vvar_menos[,5]), arr.ind = TRUE)
best_cmig_menos <- which(BICs_cmig_menos[,5] == max(BICs_cmig_menos[,5]), arr.ind = TRUE)
```
