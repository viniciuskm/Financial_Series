...
## Modelo Linear Dinâmico {.smaller}
...
```{r importando_via_var, cache=TRUE}
...
## Ações - Taxa Básica Juros SELIC

```{r importando_selic, cache=TRUE}
...
# plot(diff(log(rf)), main = 'Retorno SELIC')
...
## Calculando Beta Linear Dinâmico (Rolling Window)
...
## Calculando Beta Linear Dinâmico (Expanding Window)
...
plot(betas_exp_mat, col=1:4, lwd=2, ylim=c(0,2), main='Rolling Betas',ylab = "Beta")
abline(h=betas$Petrobras, col=1, lty=2)
abline(h=betas$Vale, col=2, lty=2)
abline(h=betas$Via_Varejo, col=3, lty=2)
abline(h=betas$Cemig, col=4, lty=2)
...
par(mfrow=c(2,2))
plot(betas_exp_mat[,1], col=1, lwd=2, 
...
plot(betas_exp_mat[,2], col=1, lwd=2, 
...
plot(betas_exp_mat[,3], col=1, lwd=2,
...
plot(betas_exp_mat[,4], col=1, lwd=2, 
...
par(mfrow=c(1,1))
ts.plot(tvp_beta_petr, col=1, ylab='Petrobras', main='Time-Varying Beta', lwd=1)
abline(h=betas$Petrobras, lty=2, lwd=2, col=1)

ts.plot(tvp_beta_vale, col=2, ylab='Vale', main='Time-Varying Beta', lwd=1)
abline(h=betas$Vale, lty=2, lwd=2, col=2)

ts.plot(tvp_beta_vvar, col=3, ylab='Via Varejo', main='Time-Varying Beta', lwd=1)
abline(h=betas$Via_Varejo, lty=2, lwd=2, col=3)

ts.plot(tvp_beta_cmig, col=4, ylab='CEMIG', main='Time-Varying Beta', lwd=1)
...
vet_back_test <- cbind(back_teste(tvp_beta_petr,
                            petr_filtered,
                            ibov_filtered,
                            MSE_petr_estatico,
                            MAE_petr_estatico,
...
vet_back_test <- cbind(back_teste(tvp_beta_vale,
                               vale_filtered,
                               ibov_filtered,
                               MSE_vale_estatico,
                               MAE_vale_estatico,
